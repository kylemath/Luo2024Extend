...
\begin{itemize}[nosep]
  ...
  \item \textbf{New Theorem~1\/$''$} (general case): For all supermodular games, a corrected bound $V_{\mathrm{Markov}}(s_1^*) \leq V(s_1^*)$ holds, with equality iff the game is belief-robust.
...
\begin{itemize}[nosep]
  ...
  \item \textbf{Theorem~1$''$} (general corrected bound): for all supermodular games, $V_{\mathrm{Markov}}(s_1^*) \leq V(s_1^*)$ holds, where $V_{\mathrm{Markov}}$ uses the state-contingent Nash correspondence $B(s_1^*, F(\cdot|\theta_t))$ averaged over the ergodic distribution.
...
\end{enumerate}
SA6 confirms empirically: $V_{\mathrm{Markov}} = \PayoffFiltered < V(s_1^*) = \PayoffStationary$, a $\PayoffOverestimation$ overestimation in the original. The gap $V(s_1^*) - V_{\mathrm{Markov}}$ is an economically meaningful object: it measures the ``cost of persistence'' in reputation games.
