...
\]
with $V_{\mathrm{Markov}} = V(s_1^*)$ if and only if the game is belief-robust. The difference $V(s_1^*) - V_{\mathrm{Markov}}$---the \emph{effect of persistence}---can be positive or negative, quantifying how state persistence affects reputation value. This is a new economic object that the i.i.d.\ framework cannot capture.
