...
These are the payoffs conditional on SR defection ($a_2 = D$). The full payoff matrix, including SR cooperation ($a_2 = C$), is:
\begin{center}
\begin{tabular}{@{}ccccc@{}}
\toprule
& \multicolumn{2}{c}{$a_2 = C$} & \multicolumn{2}{c}{$a_2 = D$} \\
\cmidrule(lr){2-3}\cmidrule(lr){4-5}
& $a_1 = A$ & $a_1 = F$ & $a_1 = A$ & $a_1 = F$ \\
\midrule
$\theta = G$ & 1 & $x$ & 1 & $x$ \\
$\theta = B$ & $y$ & $y$ & $y$ & 0 \\
\bottomrule
\end{tabular}
\end{center}
Under cooperation, $u_1(B, F, C) = y > 0 = u_1(B, F, D)$: fighting in bad states is costly against defection but acceptable against cooperation. The Stackelberg strategy $s_1^*(G) = A$, $s_1^*(B) = F$ is optimal because it is confound-defeating (monotone under supermodularity) and induces SR cooperation via reputation, not because it maximizes per-period payoffs against defection.
...
\begin{enumerate}[label=(\arabic*)]
    \item \textbf{If $x + y < 1$ (supermodular):} Under the belief-robust condition (Proposition~\ref{prop:br_condition}), a patient long-run player secures at least $V(s_1^*) = \beta/(\alpha + \beta)$ in any Nash equilibrium, for any $\mu_0 > 0$. In the general (non-belief-robust) case, the bound is $V_{\mathrm{Markov}}(s_1^*)$, which equals $V(s_1^*)$ if and only if the game is belief-robust.
...
\noindent\textbf{Worst-case commitment payoff (against defection):} Under $s_1^*(G) = A$, $s_1^*(B) = F$, if SR always defects:
\[
    V_{\min}(s_1^*) = \pi(G) \cdot u_1(G, A, D) + \pi(B) \cdot u_1(B, F, D) = 0.625 \times 1 + 0.375 \times 0 = 0.625.
\]
This is a lower bound on the long-run player's payoff regardless of SR behavior. When SR cooperates (as under i.i.d.\ beliefs with $\pi(G) = \BasePiG > \mu^* = \SRThreshold$), the i.i.d.\ equilibrium payoff is higher: $V(s_1^*) = \pi(G) \cdot u_1(G, A, C) + \pi(B) \cdot u_1(B, F, C) = \PayoffStationary$.
...
The overestimation arises because the i.i.d.\ analysis assumes SR always faces belief $\pi(G) = \BasePiG > \SRThreshold$, so SR always cooperates. Under the corrected timing (Remark~\ref{rem:timing}), SR's belief at time~$t$ is $F(\cdot|\theta_{t-1})$, not the degenerate belief at $\theta_t$. When $\theta_{t-1} = B$, SR defects because $F(G|B) = \SRBeliefAfterB < \SRThreshold$, while payoffs depend on $\theta_t$ (which may be $G$ or $B$). This reduces the LR payoff by \PayoffOverestimation\% relative to the Markov payoff.
...
Commitment payoff & $V(s_1^*)$ & $V(s_1^*)$ & $V_{\mathrm{Markov}}$ (equals $V(s_1^*)$ iff belief-robust) \\
