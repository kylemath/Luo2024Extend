...
The gap $V(s_1^*) - V_{\mathrm{Markov}}$ is a new economic object: the \emph{cost of persistence in reputation games}. It quantifies how much the long-run player loses because state persistence causes the short-run player to adjust behavior state-by-state rather than responding to time-averaged frequencies.

For the deterrence game with $\mu^* = \SRThreshold$, the cost equals $\PayoffStationary - \PayoffFiltered = \PayoffGapAbsolute$, representing \PayoffOverestimation\% of the i.i.d.\ payoff. The cost is increasing in the persistence parameter $|1-\alpha-\beta|$ and vanishes continuously as $\alpha+\beta \to 1$, providing a direct quantitative link between the dynamics of the economic environment and the value of reputation.
